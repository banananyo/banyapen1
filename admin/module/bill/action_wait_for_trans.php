		...
		<div class="col-md-8 content">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    $get = $_GET;
                        if(isset($get['id'])) {
                            $sql = "UPDATE `payment_confirm` SET `status` = 'WFT' WHERE `payment_confirm`.`id` = $id;";
                            $result = $conn->query($sql);
                            $sql = "UPDATE `orders` SET `status` = 'WFT' WHERE `orders`.`orders_ref` = '$ordersRef';";
                             // echo $sql;
                            $result = $conn->query($sql);
                            ...
                            if($result) {
                                ...
                                // $id = $ordersRef;
                                echo "<span class='is-green'>เปลี่ยนสถานะเป็น `รอโอน` ให้ $id แล้ว</span>";
                            }else {
                                echo "<span class='is-red'>เปลี่ยนสถานะเป็น `รอโอน` ให้ $id ไม่สำเร็จ</span>";
                            }
